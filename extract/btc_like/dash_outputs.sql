-- This query can be used to extract the transaction outputs data from the Dash blockchain.
--
-- Author:  Roman Overko
...
FROM `bigquery-public-data.crypto_dash.outputs` AS outputs WHERE outputs.value > 0 AND DATE(outputs.block_timestamp) < FINAL_DATE
ORDER BY date ASC
