-- This query can be used to extract the transaction inputs data from the Bitcoin Cash blockchain.
--
-- Author:  Roman Overko
...
FROM `bigquery-public-data.crypto_bitcoin_cash.inputs` AS inputs WHERE inputs.value > 0 AND DATE(inputs.block_timestamp) < FINAL_DATE
ORDER BY date ASC
